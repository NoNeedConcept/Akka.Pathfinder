...
{
    private readonly Dictionary<Directions, DirectionConfig> _directionConfigs;
    ...
    public static PointWorkerState FromInitialize(int pointId, Guid collectionId)
        => new(new Dictionary<Directions, DirectionConfig>())
        {
            PointId = pointId,
    ...
    public PointWorkerState(IReadOnlyDictionary<Directions, DirectionConfig> configs)
    {
        _directionConfigs = new Dictionary<Directions, DirectionConfig>(configs);
    }
    ...
    public IDictionary<Directions, DirectionConfig> DirectionConfigs => _directionConfigs;
    ...
    internal bool UpdateDirectionCost(DirectionCommit commit)
    {
        ...
        {
            Directions.Top => Update(commit.AdditionalCost, directionConfig, changeMethod),
            Directions.Bottom => Update(commit.AdditionalCost, directionConfig, changeMethod),
            Directions.Left => Update(commit.AdditionalCost, directionConfig, changeMethod),
            Directions.Right => Update(commit.AdditionalCost, directionConfig, changeMethod),
            Directions.Front => Update(commit.AdditionalCost, directionConfig, changeMethod),
            Directions.Back => Update(commit.AdditionalCost, directionConfig, changeMethod),
            Directions.All =>
                throw new NotImplementedException(), // Hier muss noch was gemacht werden aber ich weiss leider noch nicht wie!! @HolySafe
        ...
        static DirectionConfig Update(uint value, DirectionConfig config, ChangeMethod changeMethod) =>
            changeMethod is ChangeMethod.Increase ? config.Increase(value) : config.Decrease(value);
    }
    ...
    public void RemoveOldPathfinderIds(TimeSpan timeSpan) =>
        _inactivePathfinders.RemoveAll((_, value) => value < DateTime.UtcNow.Add(-timeSpan));
    ...
    public bool TryIsNotShortestPathForPathfinderId(FindPathRequest request)
    {
        if (request.TargetPointId.Equals(PointId)) return false;
        var currentPathCost = request.Directions.Select(x => x.Cost).Sum(x => (int)x);
        if (_pathfinderPathCost.TryGetValue(request.PathfinderId, out var value) &&
            value <= currentPathCost) return true;
        _pathfinderPathCost[request.PathfinderId] = currentPathCost;
        return false;
    }
    ...
    public IEnumerable<FindPathRequest> GetAllForwardMessages(FindPathRequest request)
    {
        var infoIds = request.Directions
            .GroupJoin(_directionConfigs, x => x.PointId, x => x.Value.TargetPointId,
                (info, points) => points.Any() ? info.PointId : int.MinValue)
            .Where(x => x != int.MinValue);

        foreach (var (key, value) in _directionConfigs.ExceptBy(infoIds, x => x.Value.TargetPointId))
        {
            var directions = request.Directions.AsEnumerable()
                .Append(new PathPoint(value.TargetPointId, value.Cost, key)).ToImmutableList();
            yield return new FindPathRequest(request.RequestId, request.PathfinderId, Guid.NewGuid(),
                value.TargetPointId, request.TargetPointId, directions);
        }
    }

    public PersistedPointWorkerState GetPersistenceState() =>
        new(PointId, CollectionId, Cost, _directionConfigs.ToDictionary(), State, Loaded);
}
