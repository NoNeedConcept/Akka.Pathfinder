    ...
    public static Dictionary<int, MapConfigWithPoints> MapConfigs => new()
        {
            { 0, Map0()},
            { 1, Map1()},
            { 2, FactoryProvider.CreateFactory().Create(new MapSettings(BaseCost, BaseCost*2, new MapSize(3, 3, 3), [], 15), true)},
            { 3, FactoryProvider.CreateFactory().Create(new MapSettings(BaseCost, BaseCost*2, new MapSize(15, 15, 15), [], 20), true)},
            { 4, FactoryProvider.CreateFactory().Create(new MapSettings(BaseCost, BaseCost*2, new MapSize(25, 25, 25), [], 20), true)},
            { 5, FactoryProvider.CreateFactory().Create(new MapSettings(BaseCost, BaseCost*2, new MapSize(35, 40, 40), [], 20), true)},
            { 6, FactoryProvider.CreateFactory().Create(new MapSettings(BaseCost, BaseCost*2, new MapSize(256, 256, 1), [], 20), true)}
        };
